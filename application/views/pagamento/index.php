		...
		<table class="tabela-1">
			...
			<tbody>
				...
				<tr>
					...
					<td style="background-color:#f9f9f9; text-align:left; font-weight:bold;">
						
						Limite de Crédito Diário
						<input type="number" style="width:100%; margin:5px 0; padding:10px 5px;" id="vl_credito" name="vl_credito" min="1" value="<?= $dados['LIMITE_DIARIO'] ?>" onkeyup="limite_diario(this, <?= $dados['CD_ALUNO']; ?>);" data-aluno="<?= $dados['CD_ALUNO']; ?>" class="input-text-25">
		...
		<div class="row">
			...
			<div class="row-60">
				...
				<select id="aluno" name="aluno" class="drop-menu">
					...
					<?php foreach($listar_aluno as $dados): ?>
					<option value="<?= $dados['CD_ALUNO']."-".$dados['NM_USUARIO']."-".$dados['PERC_DESC_ALMOCO']."-".$dados['CD_TURMA']; ?>"><?= $dados['NM_USUARIO']; ?></option>
					<?php endforeach; ?>
				</select>
...
<script>
	...
	$("#item").change(function(e){
		...
		if($(this).val() == 2647){ // credito
			html = '<div class="label" style="display:none;"><strong>Qtd.</strong><input type="hidden" id="qtd" name="qtd" value="1" readonly="readonly" class="readonly"></div><div class="label"><strong>Informe o Valor</strong><input type="text" id="preco" name="preco" value="1.00" class="preco"> </div><input type="hidden" id="descricao" name="descricao" value="<?= $listar_produto[1]['NOMEFANTASIA']?>">';
		}else{ // almoco
			html = '<div class="label"><strong>Qtd.</strong><input type="number" id="qtd" class="qtd" name="qtd" value="1" pattern="[0-9]+$" onkeyup="numero(this)"></div> <div class="label"><strong>Preço R$</strong><input type="number" id="preco" name="preco" value="" readonly="readonly" class="readonly preco_almoco"></div> <input type="hidden" id="descricao" name="descricao" value="<?= $listar_produto[0]['NOMEFANTASIA']?>">';
		}
	...
	$(document).on("change","#aluno",function(e){
		...
		console.log("RA "+desconto[0]);
		console.log("Turma "+desconto[3]);


		$.ajax({
			url: "<?= base_url('pagamento/inicio/lockfild_select'); ?>",
			type: "POST",
			data: {
				cd_usuario_alu: desconto[0]
			},
	...
	$(document).on("change","#item",function(e){
		...
		$(".preco_almoco").val(parseFloat(preco).toFixed(2));

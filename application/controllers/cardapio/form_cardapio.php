...
class Form_cardapio extends CI_Controller {
    ...
    function index() {     
        ...
        $data     = $this->cardapio->getDefaultSeamana();


        $data = array(
            'titulo_header' => 'Formulário de Cardápio',
            'listar'        => $cardapio,
            'dt_inicio'     => $data[0]['DT_INICIO'],
            'dt_fim'        => $data[0]['DT_FIM']
        );
    ...
    function update_cardapio(){
        ...
        $p_dt_ini_validade = $this->input->get_post("p_dt_ini_validade");
        $p_dt_fim_validade = $this->input->get_post("p_dt_fim_validade");

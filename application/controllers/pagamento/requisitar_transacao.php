...
class Requisitar_transacao extends CI_Controller {
    ...
    function index() {     
        ...
        try{
            ...
            $arr_aluno_log = explode(",", $produtosArray);     

            foreach($arr_aluno_log as $aluno):
                $arr_item_log = explode(":", $aluno);

                    $cpf_responsavel_log    = $arr_item_log[0];
                    $separando_log          = explode("-",$arr_item_log[1]);
                    $ra_log                 = $separando_log[0]; 
                    $nomeAluno_log          = $separando_log[1];
                    $codigoProduto_log      = $arr_item_log[2];
                    $qtdItens_log           = $arr_item_log[3];
                    $precoItem_log          = number_format($arr_item_log[4],2,",",".");
                    $totalItem_log          = $arr_item_log[5];
                    $totalCompra_log        = $arr_item_log[6];

                    $p_insert_log = array(
                        'p_tid'                     =>  $tid_log, 
                        'p_cd_usuario_resp'         =>  $cpf_responsavel_log,
                        'p_cd_usuario_cli'          =>  $ra_log,
                        'p_idprd'                   =>  $codigoProduto_log,
                        'p_prod_qtd'                =>  $qtdItens_log,
                        'p_valor_unit'              =>  $precoItem_log,
                        'p_compra_qt_parc'          =>  1,
                        ...
                        'p_cartao_nsu'              =>  $nsu_log,
                        'p_cartao_nr_autorizacao'   =>  $arp_log,
                        'p_status'                  =>  0
                    );



                   $result_insert_log = $this->pagamento->insert_ecom_vendas_produto_log($p_insert_log);
                 
            endforeach;

            if(!$result_insert_log){
                throw new Exception('Conexão com banco Invalida, tente novamente mais tarde!');
                exit;
            }
        ...
        $Codigo = "829829"; //codigo do aluno + id do produto
        ...
        //separação de linhas para inserção na sp     
        foreach($arr_aluno as $aluno):
            $arr_item = explode(":", $aluno);
                ...
                $qtdItens           = $arr_item[3];
                $precoItem          = number_format($arr_item[4],2,",","."); //str_replace(",",".",$arr_item[4]);
        ...
        if($status == 6 || $status == '6'): //compra realizada
            $p_update = array(
                'p_tid'                     => $tid_result,
                'p_cartao_nsu'              => $nsu,
                'p_cartao_nr_autorizacao'   => $arp,
